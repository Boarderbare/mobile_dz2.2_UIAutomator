...
class ChangeTextTest {
    ...
    @Test
    fun testViewTextNextActvity() {
        val packageName = MODEL_PACKAGE
        waitForPackage(packageName)
        val textLine = "New Text"
        device.findObject(By.res(packageName, "userInput")).text = textLine
        device.findObject(By.res(packageName, "buttonActivity")).click()
        device.wait(Until.hasObject(By.res(packageName, "text").enabled(true)), TIMEOUT)
        val result = device.findObject(By.res(packageName, "text")).text
        assertEquals(result, textLine)
    }
}
